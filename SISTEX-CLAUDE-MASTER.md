...
> **Documento mestre** do projeto SISTEX/YNTHEGRA. Renomear como `CLAUDE.md` no Obsidian vault.
> Concentra TODO contexto necessário para qualquer instância de Claude operar o projeto sem perda de continuidade entre sessões.
>
...
1. [O Negócio](#1-o-negócio)
2. [Os Sócios](#2-os-sócios)
3. [Arquitetura Técnica](#3-arquitetura-técnica)
4. [Infraestrutura — URLs, IDs e Credenciais](#4-infraestrutura--urls-ids-e-credenciais)
5. [Histórico Resumido (até 01/05/2026)](#5-histórico-resumido)
6. [Estado Atual do Sistema](#6-estado-atual-do-sistema)
7. [Plano de 5 Dias até Demo Stenio](#7-plano-de-5-dias-até-demo-stenio)
...
12. [Padrões de Segurança](#12-padrões-de-segurança)
13. [Lessons Learned](#13-lessons-learned)
14. [TODOs e Pendências Conhecidas](#14-todos-e-pendências-conhecidas)
15. [Comandos e Scripts Úteis](#15-comandos-e-scripts-úteis)
...
- **Plano comercial completo:** ver `precificacao-sistex-v2.html` em `C:/Users/rianf/Documents/YNTHEGRA/`
...
- **Quando um termina sessão**, deve gerar **resumo de handoff** para o outro retomar (ver seção 16).
...
- Documentos premium criados em `C:/Users/rianf/Documents/YNTHEGRA/`:
...
### Padrões de design para documentos premium (HTMLs em `Documents/YNTHEGRA/`)
...
- **Hostinger DNS é LENTO** internamente. UI mostra valor novo, mas nameserver authoritative (`byte.dns-parking.com` / `pixel.dns-parking.com`) pode demorar **30-60 minutos** pra propagar mesmo com TTL 300.
...
- **Rate limit de email:** 3-4/h no Free tier. Bloqueia signups depois de poucas tentativas. **Solução:** ativar `mailer_autoconfirm: true` via Management API ou configurar SMTP custom (Resend gratuito tem 100/dia).
...
- **`auth.users.confirmed_at` é coluna gerada** — não pode UPDATE direto. Atualizar `email_confirmed_at` resolve.
...
- **Mesma sessão de browser não permite 2 contas** — tem que usar abas anônimas pra alternar.
...
- **"This domain is linked to another Vercel account"** = remover o domínio do projeto antigo (na conta antiga) PRIMEIRO. Aí o domínio vira "DEPLOYMENT_NOT_FOUND" no Vercel global e pode ser re-adicionado em outra conta.
...
- **App "Custom" (privado por loja) vs "Public" (multi-loja)** — preferir Public com Custom Distribution: pode escalar pra múltiplos fornecedores sem revisão Shopify.
...
- Lovable Cloud cria Supabase **na própria org Lovable** — quando você "sai" do Lovable, perde acesso ao Supabase.
...
SISTEX-CLAUDE-MASTER.md               — Este documento
```

### Arquivos relevantes em `C:/Users/rianf/.claude/projects/.../memory/`

```
MEMORY.md                              — Índice das memórias
user_context.md                        — Quem é o usuário
sistex_project.md                      — Visão geral
sistex_infrastructure.md               — Infra do projeto
ynthegra_platform_model.md             — Modelo multi-tenant
project_ferstex_accounts.md            — Ferstex já tem Bling/ME/Shopify
project_core_value_prop.md             — Motor de Produção é a dor #1
project_retention_strategy.md          — SISTEX = infraestrutura, não opcional
feedback_simplicity.md                 — UX simplicidade
stenio_ferstex_relationship.md         — Relação com Stenio
session_handoff_protocol.md            — Protocolo de handoff Rian↔Erick
```
...
> **Próximo Claude que ler este documento:** você tem 100% do contexto. Não precisa recomeçar do zero.
